...
export interface EditMigratedDesignAndContractParams {
...
export interface EditMigratedDesignAndContractForm {
    id: number | null;
...
export class EditMigratedDesign {
    static createEmptyForm(): EditMigratedDesignAndContractForm {
        return {
            id: null,
            approvalNumber: null,
            documentNumber: null,
    ...
    static contractAndDesignToForm(
        design: Design,
    ): EditMigratedDesignAndContractForm {
        const contract = design.contracts[0];
        const lastContractedPrice =
            design.contracts.reverse()[0].contractedPrice;
        return {
            id: contract.id,
            approvalNumber: contract.approvalNumber,
            // documentNumberだけconstructionに紐づく
            documentNumber: design.documentNumber?.value || null,
    ...
    static formToContractParams(
        form: EditMigratedDesignAndContractForm,
        ...
        return {
            ...
            // 基本的にEditMigratedDesignAndContractParamsはcontracts.firstを更新するparamsなので、contracted_priceにfirstContractedPriceを入れる
