...
export const SupplierCsvModal: FC<ModalProps> = ({ onHide, ...modalProps }) => {
    ...
    const onSubmit: FormEventHandler = async e => {
        e.preventDefault();
        csvFile && (await SupplierRepository.storeCsv(csvFile));
        location.reload();
    };
    ...
    return (
        <Modal {...modalProps} onHide={onHide}>
            <Modal.Header closeButton>
                <Modal.Title>業者マスタ CSV 一括登録</Modal.Title>
            </Modal.Header>
            <Modal.Body>
                <form onSubmit={onSubmit}>
                    <Form.Group>
                        <Form.Label>
                            指定された形式の CSV で業者マスタを一括登録します
                        </Form.Label>
                        <input
                            required
                            type='file'
