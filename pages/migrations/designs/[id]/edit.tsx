...
import {
    EditMigratedDesign,
    EditMigratedDesignAndContractForm,
} from '@/App/Service/EditMigratedDesign';
...
const EditMigratedDesignPage: NextPage<Props> = props => {
    const router = useRouter();
    const { users, suppliers } = useContext(MasterContext);
    const [form, setForm] = useState<EditMigratedDesignAndContractForm>(
        EditMigratedDesign.createEmptyForm(),
    );
    const [design, setDesign] = useState<Design>();

    const fetchData = async () => {
        const fetchedDesign = await DesignRepository.get(props.id);
        setDesign(fetchedDesign);
        const newForm =
            EditMigratedDesign.contractAndDesignToForm(fetchedDesign);
        setForm(newForm);
    };
    ...
    const onSubmit: FormEventHandler = async e => {
        e.preventDefault();
        const params = EditMigratedDesign.formToContractParams(form);
        const res =
            await MigratedDesignRepository.updateMigratedDesignAndContract(
                props.id,
                params,
            );
        res && alert('更新しました');
        router.push(`/designs/${props.id}/summary`);
    };

    return (
        <Page>
            <Navbar bg='white' className='px-5'>
                <Link href={`/migrations/designs/${props.id}`} passHref>
...
export default EditMigratedDesignPage;
